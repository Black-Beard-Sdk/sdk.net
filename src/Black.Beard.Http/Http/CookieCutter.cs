...
namespace Bb.Http
{
    ...
    public static class CookieCutter
    {
        ...
        public static bool IsValid(this UrlCookie cookie, out string reason)
        {
            ...
            //if (!string.IsNullOrEmpty(Path) && !Path.OrdinalStartsWith(Url.Slash)) {
        ...
        private static bool IsPathMatch(this UrlCookie cookie, Url requestUrl, out string reason)
        {
            ...
            string GetDefaultPath()
            {
                ...
                if (origPath == string.Empty || origPath[0] != '/') return Url.Slash;
                if (origPath.Count(c => c == '/') <= 1) return Url.Slash;
                return origPath.Substring(0, origPath.LastIndexOf('/'));
            }

            // https://tools.ietf.org/html/rfc6265#section-5.2.4
            var cookiePath = (cookie.Path?.OrdinalStartsWith(Url.Slash) == true) ? cookie.Path : GetDefaultPath();

            if (cookiePath.Length > 1 && cookiePath.OrdinalEndsWith(Url.Slash))
                cookiePath = cookiePath.TrimEnd('/');

            if (cookiePath == Url.Slash)
                return true;

            var requestPath = (requestUrl.Path.Length > 0) ? requestUrl.Path : Url.Slash;
